...
exports.executeQuery = exports.queryResultIsShowResult = exports.queryResultToText = exports.queryResultToObjectsArray = exports.pollQueryCompletedState = exports.getOutputBucketName = exports.getDatabaseName = undefined;
...
let queryResultToObjectsArray = exports.queryResultToObjectsArray = function (queryResult) {
  ...
  let rowsObjects = _lodashFirecloud2.default.map(rows, function (row) {
    ...
    _lodashFirecloud2.default.forEach(columns, function (column, columnIndex) {
      let value = _lodashFirecloud2.default.get(row, `Data[${columnIndex}].VarCharValue`);
...
let queryResultToText = exports.queryResultToText = function (queryResult) {
  let rows = queryResult.ResultSet.Rows;
  let lines = _lodashFirecloud2.default.map(rows, 'Data[0].VarCharValue');
  return _lodashFirecloud2.default.join(lines, '\n');
};

let queryResultIsShowResult = exports.queryResultIsShowResult = function (queryResult) {
  let showColumnSets = [['createtab_stmt'], ['tab_name'], ['database_name'], ['partition'], ['field'], ['prpt_name', 'prpt_value']];

  let columnInfo = queryResult.ResultSet.ResultSetMetadata.ColumnInfo;

  return _lodashFirecloud2.default.some(showColumnSets, function (columnSet) {
    return columnInfo.length === columnSet.length && _lodashFirecloud2.default.every(columnInfo, function (column) {
      return column.Name === column.Label && column.Type === 'string' && _lodashFirecloud2.default.includes(columnSet, column.Name);
    });
  });
};
