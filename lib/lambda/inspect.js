...
let inspect = async function ({ ctx }) {
  ...
  for (let key in inspection) {
    await (async createError => {try {return await ctx.log.trace({ _babelSrc: { file: typeof __filename === "undefined" ? "src/lambda/inspect.js" : __filename, babelFile: "src/lambda/inspect.js", line: 80, column: 11 } }, `Inspecting '${key}'`, {
          key: inspection[key] });} catch (_awaitTraceErr) {let err = createError();_awaitTraceErr.stack += "\n...\n" + err.stack;throw _awaitTraceErr;}})(() => new Error());
